...
class TigerDriver(object):
    ...
    def getToken(self, secret, setToken=True, lifetime=None) -> tuple:
        ...
        """
        res = json.loads(requests.request("GET", "/requesttoken?secret=" + secret + ("&lifetime=" + str(lifetime) if lifetime else "")).text)
        if not res["error"]:
            if setToken:
        ...
        if "Endpoint is not found from url = /requesttoken" in res["message"]:
            raise TigerDriverException("REST++ authentication is not enabled, can't generate token.", None)
        raise TigerDriverException(res["message"], (res["code"] if "code" in res else None))
    ...
    def refreshToken(self, secret, token=None, lifetime=2592000) -> tuple:
        ...
        if not token:
            token = self.apiToken
        res = json.loads(requests.request("PUT", "/requesttoken?secret=" + secret + "&token=" + token + ("&lifetime=" + str(lifetime) if lifetime else "")).text)
        if not res["error"]:
            exp = time.time() + res["expiration"]
            return res["token"], int(exp), datetime.utcfromtimestamp(exp).strftime('%Y-%m-%d %H:%M:%S')
        if "Endpoint is not found from url = /requesttoken" in res["message"]:
            raise TigerDriverException("REST++ authentication is not enabled, can't refresh token.", None)
        raise TigerDriverException(res["message"], (res["code"] if "code" in res else None))
    ...
    def deleteToken(self, secret, token=None, skipNA=True) -> bool:
        ...
        if not token:
            token = self.apiToken
        res = json.loads(requests.request("DELETE", "/requesttoken?secret=" + secret + "&token=" + token).text)
        if not res["error"]:
            return True
        if res["code"] == "REST-3300" and skipNA:
            return True
        if "Endpoint is not found from url = /requesttoken" in res["message"]:
            raise TigerDriverException("REST++ authentication is not enabled, can't delete token.", None)
        raise TigerDriverException(res["message"], (res["code"] if "code" in res else None))
